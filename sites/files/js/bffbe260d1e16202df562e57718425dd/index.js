...
;Drupal.locale = { 'pluralFormula': function($n) { return Number(($n!=1)); }, 'strings': { "Select all rows in this table": "Seleccionar todas las filas de esta tabla", "Deselect all rows in this table": "Quitar la selección a todas las filas de esta tabla", "Your server has been successfully tested to support this feature.": "Su servidor pasó con éxito la prueba sobre soporte de esta característica.", "Upload": "Upload", "Only files with the following extensions are allowed: %files-allowed.": "Sólo se permiten archivos con las siguientes extensiones: %files-allowed.", "An HTTP error @status occurred. \n@uri": "Ocurrió un error HTTP @status. @uri" } };;/*
...
{returnColour=colour;}}return returnColour;}function strip_px(value){if(typeof(value)!='string')return value;return parseInt(((value!="auto"&&value.indexOf("%")==-1&&value!=""&&value.indexOf("px")!==-1)?Math.round(value.slice(0,value.indexOf("px"))):0))}};})(jQuery);;// $Id: tableheader.js,v 1.16.2.1 2008/10/01 23:30:36 goba Exp $
...
;Drupal.behaviors.palettetheme = function (context) {
//do some fancy stuff
//    $(document).pngFix();
    //$(".lavaLampmenu").lavaLamp({ fx: "backout", speed: 1200 }); 
    //$("#lavaLampVariableImage").lavaLamp({fx: "easeOutSine", speed: 800, startItem: 1 });
    //	$('#lavaLampBasicImage').lavaLamp({ fx: 'easeInExpo', speed: 800, homeTop:-1, homeLeft:-1});
    // $("#1, #2, #3").lavaLamp({ fx: "backout", speed: 700, click: function(event, menuItem) { return false; } });
  $(function() { $(".lavaLamp").lavaLamp({ fx: "backout", speed: 700 })});
  $('#menuheader li.active').addClass('current');
  //$('#menuheader li.active-trail').click();
  // este es el que funciona!
  $('#menuheader li.active').click();
  //$('#menuheader li.active').css('border','1px solid red');
  $('.blog-content').corner(); 
};
;